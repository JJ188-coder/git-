(function () {
  ...
  function qualityLabel(quality) {
    if (!quality?.scoredSegmentCount || quality.meanConfidence == null) return "等待可量化数据";
  ...
  function qualityCard(label, quality) {
    ...
    const confidence = quality?.meanConfidence == null ? "—" : `${Math.round(quality.meanConfidence * 100)}%`;
  ...
  async function action(value) {
    try {
      ...
      else if (value === "start") { const c = course(); if (!c) return; const lecture = await api("/api/lectures/start", { method: "POST", body: JSON.stringify({ courseID: c.id }) }); state.currentLectureID = lecture.id; state.currentCourseID = c.id; state.detail = await api(`/api/lectures/${lecture.id}`); toast("课堂已开始；关闭网页也会继续录音"); await reload(); }
